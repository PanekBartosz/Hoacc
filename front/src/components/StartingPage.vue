...
watch(
  ...
  () => {
    ...
    isVisible.value =
      initialIsVisible !== null ? JSON.parse(initialIsVisible) : true;
  }
);
      ...
      <div class="text-right xxs:mt-5 flex justify-end">
        <div class="flex items-center md:mr-[50px]">
          <button
            ...
            class="text-[16px] mt-7 xxs:mt-0 sm:mr-6 mr-3 font-bold sm:text-[32px]"
          >
            HOME
          </button>
          <button
            @click="navigateToAbout"
            :class="{ 'text-white': isActiveRoute('About') }"
            class="text-[16px] mt-7 xxs:mt-0 sm:mr-6 mr-3 font-bold sm:text-[32px]"
          >
            ABOUT US
          </button>
